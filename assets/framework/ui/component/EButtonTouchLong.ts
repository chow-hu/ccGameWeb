/**
 * Name = EButtonTouchLong
 * Time = Thu Apr 07 2022 13:02:05 GMT+0800 (中国标准时间)
 * Author = aby
 * 长按按钮点击(触摸事件)
 */
import { Component, EventHandler, EventTouch, Node, Rect, UITransform, Vec3, _decorator, error } from "cc";

const { ccclass, property, menu } = _decorator;

/** EButtonTouchLong 回调时的状态 */
export const EButtonTouchLongStatus = {
    /** 开始点击(只有长按才有) */
    START: "START",
    /** 运行时(只有长按才有) */
    RUN: "RUNNING",
...
@ccclass("EButtonTouchLong")
@menu('UI/EButtonTouchLong')
export class EButtonTouchLong extends Component {
    @property({
    ...
    update(dt: number) {
        if (this._currowEvent && this.interactable == true && this._isCanCallback == true) {
            ...
            if (this.touchlongtime <= 0) {
                this._isTouchLong = false;
                this.onClickCallback(this._currowEvent, EButtonTouchLongStatus.END);
                this.removeTouchLong();
                return;
            }
            //以下长按
            if (this._currowPassTime >= this.touchlongtime) {
                let status = EButtonTouchLongStatus.RUN;
                if (this._isTouchLong == false) {
                    status = EButtonTouchLongStatus.START;
                }
                if (this.onceLongCallback == true) {
                    status = EButtonTouchLongStatus.END;
                }
