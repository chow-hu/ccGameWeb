import { Component, Layout, Mask, Node, NodePool, Rect, ScrollView, Size, UITransform, Vec2, Vec3, _decorator, tween, v3 } from 'cc';
import { ui2d } from '../../ge';
...
export default class ScrollOption extends Component {
    ...
    private aScrollDectoin: number = 0; // 0 向左 1 向右 
    private oldOffsetX: number = 0;
     ...
     * @param p 
     */
    init(p: InitParam, callback: Function) {
        if (callback) {
    ...
    private _init(p: InitParam) {
        ...
        if (this._scrollView.vertical) {
            ...
            // this.middleItemRect = new Rect(-sellSize[0] / 2, -nodeSize.height / 2, sellSize[0] * 1.4, nodeSize.height);
     ...
     */
    private _load(keepPos: boolean = false) {
    ...
    private _scrollEnded() {
        this._scrollView.bounceDuration
        if (!this._inited) return;
        ...
        if (this.aScrollDectoin == 0) {
            deltaOffset = itemSize * 0.4;
        } else if (this.aScrollDectoin == 1) {
            deltaOffset = - itemSize * 0.4;
        }
        if (endPos <= (itemSize + spacing) / 2 + deltaOffset) {
            this._scrollView.scrollToLeft(0.15);
            this._updateScrollView();
            return;
        }
        // @ts-ignore
        const maxOffset: Vec3 = this._scrollView.getMaxScrollOffset();
        const bottom: number = maxOffset.x - (itemSize + spacing) / 2 + deltaOffset;
        if (endPos > bottom) {
            this._scrollView.scrollToRight(0.15);
            this._updateScrollView();
            return;
        ...
        for (let i = 1, len = this._delegate.getCellNumber() - 1; i < len; i++) {
            const h: number = itemSize + spacing;
            if (endPos <= offset + h + deltaOffset) {
                offset += h / 2;
    ...
    private _updateScrollView() {
        if (!this._inited) return;
        // console.log("updateScrollView-----------------offset:" + this._scrollView.getScrollOffset().x);
        // console.log("updateScrollView-----------------offset:" + this._scrollView.content.getComponent(UITransform).contentSize.width);

        if (this.updateCallback) {
            // this.updateCallback(this._scrollView.getScrollOffset().x);
        }
        let offet = Math.abs(this._scrollView.getScrollOffset().x);
        if (offet - this.oldOffsetX > 0) {
            this.aScrollDectoin = 1;
        } else {
            this.aScrollDectoin = 0;
        }
        this.oldOffsetX = offet;
        // let scrollOffset = event.getScrollOffset();
        // if (scrollOffset.x < this._scrollView.getScrollOffset().x) {
        //     this.aScrollDectoin = 0;
        // } else if (scrollOffset.x > this._scrollView.getScrollOffset().x) {
        //     this.aScrollDectoin = 1;
        // }
    ...
    handlePos(node: Node) {
        let box = node.getComponent(UITransform).getBoundingBoxToWorld();
        let view = this.node_content.parent;
        let position = view.worldPosition;
        let size = ui2d.size(view);
        let boxPatent = new Rect(position.x - size.width / 2, position.y - size.height / 2, size.width, size.height);

        let aNode = node.getChildByName("Node");
        if (aNode) {
            aNode.active = box.intersects(boxPatent);
        }
    }
    /**
     * 构建cell节点，绑定数据
    ...
    private _addCellView(dataIndex: number): ScrollOptionCell {
        ...
        if (!cell) {
            ...
            cell.node.on(Node.EventType.TRANSFORM_CHANGED, (type) => {
                if (type & Node.TransformBit.POSITION) {
                    this.handlePos(cell.node);
                }
            }, this);
        }
        ...
        cell.actived = false;
        // tween(cell.node.getChildByName("Node").getComponent(UIOpacity)).to(0.3, { opacity: 255 }).start();
        tween(cell.node.getChildByName("Node")).to(0.3, { scale: v3(1, 1, 1) }).start();
        const callback = () => {
            if (cell.actived)
                return;
            if (this._scrollView.vertical) {
    ...
    public handleSomeRefresh() {
        this.node_content.setPosition(0, 0, 0);
        let dataLen = this._delegate.getCellNumber();
        for (let i = 0; i < dataLen; i++) {
            let cell: ScrollOptionCell = this._getCellViewFromPool();
            if (!cell) {
                cell = this._delegate.getCellView(i);
                this.handlePos(cell.node);
            }
