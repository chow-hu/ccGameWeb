...
let modifyBuildTemplate = function (name, orientation, id) {
    ...
    } else if (id == 110 || id == 109 || id == 111 || id == 112) {
...
let backupApk = function (build_env, bundle, id) {
    ...
    let date = new Date();
    dst = path.join(__dirname, `../ccgamePack/game/${platorm}/${build_env}/${getVersion()}/${id}-${name}-(v${getGameVersion(id)})-(${date.getFullYear()}${addZero(date.getMonth() + 1)}${addZero(date.getDate())}_${addZero(date.getHours())}${addZero(date.getMinutes())}${addZero(date.getSeconds())})`);//${platorm}/
    console.log(dst);
