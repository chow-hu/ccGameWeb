...
export class CountTime {
    ...
    protected Update(dt = null) {
        if (this._isDown) {
            ...
            //更新中
            if (this._updateCall) {
                this._updateCall(this._currowNum, this._callParam, this._isFirstCall);
                if (this._isFirstCall) { this._isFirstCall = false; }
            }
            //停止了
            if (this._currowNum <= this._endTime) {
                ...
                this.Stop();
                if (this._stopCall) {
                    this._stopCall(this._currowNum, this._callParam, this._isFirstCall);
                    if (this._isFirstCall) { this._isFirstCall = false; }
                }
            }
        } else {
