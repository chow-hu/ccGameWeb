 ...
 * @Reference: 
 */
import { CCInteger, CCObject, Enum, Node, Prefab, ScrollView, Widget, _decorator } from 'cc';
...
import { TileListView } from '../helper/tilelistview';
...
@ccclass('ItemSettingInfo')
export class ItemSettingInfo extends CCObject {
    @property({ tooltip: '距滚动条头部的距离' })
    padding_begin: number = 0;
    @property({ tooltip: '距滚动条尾部的距离' })
    padding_end: number = 0;
    @property({ type: CCInteger, tooltip: '行间隔' })
    gap_x: number = 0;
    @property({ type: CCInteger, tooltip: '列间隔' })
    gap_y: number = 0;
    @property({tooltip: '是否自动排列设置column和row' })
    auto_arrange: boolean = false;
    @property({
        type: CCInteger, tooltip: '垂直方向排版时，水平方向上的列数',
        visible: function (this: ItemSettingInfo) {
            return this.auto_arrange == false;
        }
    })
    column: number = 0;
    @property({
        type: CCInteger, tooltip: '水平方向排版时，垂直方向上的行数',
        visible: function (this: ItemSettingInfo) {
            return this.auto_arrange == false;
        }
    })
    row: number = 0;
    // @property({ type: CCInteger, tooltip: '前后各缓存多少数量' })
    // cacheNum: number = 0;
}

@ccclass('funcNames')
export class IFuncNames extends CCObject {
    @property({ tooltip: 'item更新setter' })
    item_setter: string = '';
    @property({ tooltip: '单个item的设置(在数据items拼装前调用具体的回调)' })
    item_preSetter: string = '';
    @property({ tooltip: '回收时的回调' })
    recycle_cb: string = '';
    @property({ tooltip: 'item选中回调' })
    select_cb: string = '';
    @property({ tooltip: 'item选中效果setter' })
    select_setter: string = '';
    @property({ tooltip: '滚动到左边的回调' })
    scroll_to_left_cb: string = '';
    @property({ tooltip: '滚动到尽头的回调' })
    scroll_to_end_cb: string = '';
    @property({ tooltip: '滚动到开头的回调' })
    on_scroll_to_top_cb: string = '';
}


@ccclass('ETileListView')
@menu('自定义组件/ETileListView')
export class ETileListView extends UIComponentBase {
    private _list?: TileListView
    private _params?: ListViewParams<TileNode>
    private _data?: any[]
    ...
    @property({
        type: Node,
        tooltip: '可复制的子节点',
        visible: function (this: ETileListView) {
    ...
    private _loadList(keepPos?: boolean, effect = true) {
        ...
        if (!this._list || !this._list.isAlive) {
            this._list = new TileListView(this._params);
        }
