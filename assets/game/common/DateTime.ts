...
export namespace DateTime {
     ...
     */
    export function timeFormat(timestamp: number, fmt: string): string {
        if (timestamp < 10000000000) timestamp *= 1000; // 转毫秒
        let toTimeZone = DateTime.TimeZoneDefault();
        //计算时间偏差
        let offset = DateTime.getTargetTimeZoneOffset(toTimeZone);
        // 创建对应的Date对象
