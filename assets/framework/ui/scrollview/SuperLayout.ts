import { Component, EventHandler, Node, PageViewIndicator, Prefab, Size, UITransform, Vec2, Vec3, Widget, _decorator, ccenum, instantiate } from 'cc';
import { ISuperSrollowView } from './ISuperSrollowView';
...
/**
 * Name = SuperLayout
 * URL = db://assets/framework/ui/scrollview/SuperLayout.ts
 * Time = Mon Feb 27 2023 09:54:35 GMT+0800 (中国标准时间)
 * Author = aby
 * SuperScrollView的内容组件 切记锚点一定要为(0.5，0.5)
...
//方便编辑器识别的菜单项目
@menu('UI/scrollview/SuperLayout')
export class SuperLayout extends Component {
    ...
    @property({ tooltip: "绑定了SuperScrollView的ScrollowView", type: ISuperSrollowView })
    scrollView!: ISuperSrollowView;
    ...
    @property({
        ...
        visible: function (this: SuperLayout) {
    ...
    @property({ tooltip: "最小值=1，大于1就是Grid九宫格模式" }) groupItemTotal: number = 1;
    @property({ tooltip: "决定最多创建Prefab的倍数 默认2倍" }) multiple: number = 2;
    ...
    @property({ type: EventHandler, tooltip: "子节点刷新的回调" })
    ...
    /** 边缘数据（世界坐标） */
    public sideWorldArr: { viewLeftSide?: number, viewRightSide?: number, viewBottomSide?: number, viewTopSide?: number } = {};
    ...
    onLoad() {
        ...
        // Object.getPrototypeOf(this.transform)["get" + "contentSize"] = () => this.contentSize;
        // Object.getPrototypeOf(this.transform)["get" + "width"] = () => this.contentSize.width;
        // Object.getPrototypeOf(this.transform)["get" + "height"] = () => this.contentSize.height;
    ...
    private refreshSize() {
        ...
        //显示区域左边缘
        let viewLeftSide = this.view.node.worldPosition.x - (this.view.width * this.view.anchorX);
        //显示区域右边缘
        let viewRightSide = this.view.node.worldPosition.x + (this.view.width * (1 - this.view.anchorX));
        //显示区域下边缘
        let viewBottomSide = this.view.node.worldPosition.y - (this.view.height * (1 - this.view.anchorY));
        //显示区域上边缘
        let viewTopSide = this.view.node.worldPosition.y + (this.view.height * this.view.anchorY);
        this.sideWorldArr = {
     ...
     */
    setTotal(count: number, refreshLastItem: boolean = false): this {
        if (!this._isOnLoad) {
            this._itemTotal = count;
            return;
        }
        // console.warn("SuperLayout:更新item数量:", count)
        this.scrollView.canTouchMove = false
        this.currentCreateItemTotal = count;

        let oldCount = this.node.children.length;
     ...
     */
    updateItems(): this {
        // console.log("刷新所有item===>", this.headerIndex)
    ...
    private resetIndexStartToEnd(index: number) {
        // console.log("重置从头到尾的索引===========>start", index)
        ...
        // console.log("重置从头到尾的索引===========>end", index)
    }
    ...
    scrollToIndex(index: number, timeInSecond?: number, boundary?: Vec3, reverse: boolean = false) {
        ...
        let child = this.node.children.find((item: any) => item["__index"] == index)
        let nearFooter = this.isNearFooter(index)
        ...
        if (!child) {
            ...
            let flag = this.vertical && this.verticalAxisDirection == VerticalAxisDirection.TOP_TO_BOTTOM || !this.vertical && this.horizontalAxisDirection == HorizontalAxisDirection.LEFT_TO_RIGHT
            if (nearFooter) {
    ...
    protected soonFinish() {
        ...
        for (let i = 0; i < this.node.children.length; i++) {
            ...
            let multiple = this.getCenterAnchor(child.getComponent(UITransform)!, this.centerPosition)
    ...
    protected refreshItems(offset: number, refreshLastItem: boolean = false) {
        // console.log("输出偏移：", offset)
        if (offset < 0) {
            for (let i = 0; i < -offset; i++) {
                if (this.headerLoop) {
                    // console.log("插入头 输出I:", i)
                    this.pushToHeader()
                } else if (this.footerLoop) {
                    // console.log("插入尾 输出I:", i)
                    this.pushToHeader()
                } else {
                    // console.log("插入头尾 输出I:", i)
                    this.pushToHeader(true)
                    this.pushToFooter()
                }
            }
    ...
    protected createItems(count: number, refreshLastItem: boolean = false) {
        ...
        if (this.node.children.length > count) {
            // console.warn("有多余的item 需要删除 不处理====>", this.node.children.length, count)
        ...
        if (!this.needAddPrefab()) {
            ...
            if (this._maxPrefabTotal > 0 && this._maxPrefabTotal == this.node.children.length) {
                // console.warn("已经固定item总数 不处理====>", this.node.children.length, this._maxPrefabTotal)
                return
            }
        }
        let total = count - this.node.children.length //计算当前应该创建的总数
        // console.warn("计算当前应该创建的总数===>", total)
        for (let i = 0; i < total; i++) {
            let child: any = this.spawnNode();
            const transform = child.getComponent(UITransform);
            let reverse = false;
            let index = this.node.children.length - 2;
            let relative;
            if (child["__index"] == 0) {
            ...
            child.on(Node.EventType.SIZE_CHANGED, () => {
                // console.log("节点尺寸改变"); this.onChangeChildSize(transform) 
            }, this, true)
            child.on(Node.EventType.TRANSFORM_CHANGED, (type: any) => {
                // console.log("大小宽高改变"); this.onChangeChildScale(type, transform) 
            }, this, true)
            // this.notifyRefreshItem(child)
            ...
            if (!this.needAddPrefab()) {
                ...
                // console.log("SuperLayout:已固定item数量", this._maxPrefabTotal)
                break
            }
        }
    }
    /** 获取一个新的节点 */
    protected spawnNode(): Node {
        let child: Node = this._node_pool.shift();
        if (!child) {
            let node = this.cloneNode ? this.cloneNode : this.clonePrefab;
            //@ts-ignore
            child = instantiate(node);
        }
        const transform = child.getComponent(UITransform);
        this.setAndSaveSizeAndScale(transform);
    ...
    /** 重新计算当前所有Item的位置 */
    public resetChilds(start: boolean = false) {
    ...
    public pushToFooter(force: boolean = false) {
        if (this.vertical) {
            if (this.verticalAxisDirection == VerticalAxisDirection.TOP_TO_BOTTOM) {
    ...
    public pushToHeader(force: boolean = false) {
        if (this.vertical) {
            if (this.verticalAxisDirection == VerticalAxisDirection.TOP_TO_BOTTOM) {
    ...
    protected notifyRefreshItem(target: Node) {
        // console.log("通知给定的node刷新数据=============>", (target as any)['__index'])
