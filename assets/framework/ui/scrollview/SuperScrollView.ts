...
import { EventHandler, EventTouch, Node, PageView, Rect, ScrollView, UITransform, Vec2, Vec3, Widget, _decorator } from 'cc';
 ...
 * URL = db://assets/framework/ui/scrollview/SuperScrollView.ts
 * Time = Mon Feb 27 2023 09:54:35 GMT+0800 (中国标准时间)
 * Author = aby
 * 超级滚动容器 支持无限循环 复用子节点(无法精准监听边缘事件)
 * Life: onLoad-->onEnable->start->update->lateUpdate->onDisable->onDestroy
 */

@ccclass('SuperScrollView')
//方便编辑器识别的菜单项目
@menu('UI/scrollview/SuperScrollView')
export class SuperScrollView extends ISuperSrollowView {
    ...
    /** 真实的View */
    private realView: UITransform = null;
    /**边缘数据 */
    public sideArr: { viewLeftSide?: number, viewRightSide?: number, viewBottomSide?: number, viewTipSide?: number, } = {};
    ...
    onLoad() {
        this.realView = this.content.parent.getComponent(UITransform);
    ...
    private refreshSize() {
        ...
        if (this.realView.node.getComponent(Widget)) {
            this.realView.node.getComponent(Widget).updateAlignment();
        }
        //显示区域左边缘
        let viewLeftSide = this.realView.node.worldPosition.x - (this.realView.width * this.realView.anchorX);
        //显示区域右边缘
        let viewRightSide = this.realView.node.worldPosition.x + (this.realView.width * (1 - this.realView.anchorX));
        //显示区域下边缘
        let viewBottomSide = this.realView.node.worldPosition.y - (this.realView.height * (1 - this.realView.anchorY));
        //显示区域上边缘
        let viewTipSide = this.realView.node.worldPosition.y + (this.realView.height * this.realView.anchorY);
        this.sideArr = {
    ...
    get layout() {
        if (!this.superLayout) { this.superLayout = this.content?.getComponent(SuperLayout)! }
    ...
    protected _onTouchMoved(event: EventTouch, captureListeners: any) {
        ...
        if (this.isTransmitEvent) {
            if (this.direction == ScrollViewDirection.NONE) {
                ...
                let curre = event.getLocation()
                let xOffset = Math.abs(start.x - curre.x)
                let yOffset = Math.abs(start.y - curre.y)
                if (xOffset > yOffset) {
            ...
            let canTransmit = (this.vertical && this.direction === ScrollViewDirection.HORIZONTAL) || this.horizontal && this.direction == ScrollViewDirection.VERTICAL
            if (canTransmit) {
    ...
    _startAutoScroll(deltaMove: any, timeInSecond: any, attenuated: any, flag: boolean = false) {
    ...
    protected _autoScrollToPage() {
        ...
        if (bounceBackStarted) {
            ...
            let timeInSecond = this.layout.pageTurningSpeed * Math.abs(index - nextIndex)
    ...
    protected transmitEvent(event: EventTouch, eventType: string) {
        let e = new EventTouch(event.getTouches(), event.bubbles, eventType);
        e.type = eventType;
        e.touch = event.touch
